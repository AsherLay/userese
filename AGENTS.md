...
5. 涉及目录结构时读取 `docs/adr/`。

## 当前事实源

- 当前项目运行行为：`SKILL.md`、`references/`、`scripts/`、`tests/` 为 `0.3.0`。
...
- Userese 与配套 Writer（`userese-writer-qwen3-8-flash`、`userese-writer-gemini3-7-flash`）以 MIT 协议发布。提交、推送、打 tag 和可见性变更仍须用户确认当前差异后执行。
